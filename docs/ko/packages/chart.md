...
아래 미리보기에는 기본 제공 차트 여섯 가지가 모두 들어 있습니다. 직교 좌표 차트에서는 선택뿐 아니라 현재 보이는 범위를 옮기고 확대·축소하거나 원래 범위로 되돌리는 동작도 확인할 수 있습니다. 파이와 도넛은 전체에서 각 부분이 차지하는 비율을 비교하는 데 집중합니다.

<ChartPackageExample />

**사용 코드**는 페이지 상단의 **연결 방식**을 따릅니다. 미리보기에는 문서용 배치와 표현이 추가되어 있으며, 사용 코드는 공개 Vue 구성, DOM 연결, 화면 환경과 무관한 Chart 사용 흐름에 집중합니다.
