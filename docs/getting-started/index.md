...
This Checkbox is intentionally small, but it is the same example system used throughout the component catalog. Switch the host in the example frame to inspect the Vue or DOM source, then open **Code** and copy the version you need.

<ComponentExample component="checkbox" scenario="binary" title="Checkbox" description="Toggle one optional setting with a label and visible checked state." :index="0" />

Sectile supplies the behavior and accessibility state. The example CSS is only one possible presentation; replace it with your own design system.
