...
function componentApiIndex(korean = false) {
  ...
  const intro = korean
    ? '속성, 이벤트, 슬롯, 공개 타입, 파트 선택자와 키보드 동작을 컴포넌트별로 확인합니다. 각 컴포넌트 페이지에서는 상호작용 미리보기와 연결 방식별 사용 코드를 함께 볼 수 있습니다.'
    : 'Look up props, events, slots, public types, part selectors, and keyboard behavior by component. Each component page pairs an interaction preview with host-specific usage code.';
