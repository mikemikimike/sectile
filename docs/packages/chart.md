...
The preview includes every built-in chart type. Cartesian charts expose selection plus visible pan, zoom, and reset controls; radial charts focus on comparing parts of a total.

<ChartPackageExample />

The **Usage code** follows the Integration selected in the page header. The preview adds documentation layout and presentation, while the code view focuses on the public Vue, DOM, or renderer-neutral chart flow.
