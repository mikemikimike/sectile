...
import type { MachineUpdate, RevisionResult, RevisionSnapshot } from '@sectile/core/revision';
...
export function createTerminalPopup<State, Event, Command extends object>(options: Options<State, Event, Command>): Result<TerminalPopupConnection<State, Event>> {
  const runtime = createControlledComponentController<State, Event, Command, boolean>({ controlled: options.controlled, initial: options.initial, reducer: options.reducer, create: options.create, read: options.read, ...(options.command === undefined ? {} : { publishEffect: options.command }), onChange: (open) => options.onOpenChange?.(open), ...(options.interaction === undefined ? {} : { interaction: options.interaction }) });
...
class TerminalPopup<State, Event, Command extends object> implements TerminalPopupConnection<State, Event> {
  ...
  public handleEvent(event: Event): boolean {
    ...
    let result: RevisionResult<State, Command>;
    try {
