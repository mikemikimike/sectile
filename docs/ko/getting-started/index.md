...
재사용 가능한 선택자는 [스타일 적용](/ko/guides/styling)에서, 열림·닫힘·표시자·스와이프 애니메이션은 [모션](/ko/guides/motion)에서 설명합니다.

## 5. 상태를 직접 관리해야 할 때

대부분의 컴포넌트는 내부 상태로 시작할 수 있습니다. 애플리케이션이 기준 상태를 직접 가져야 한다면 각 컴포넌트의 상태 제어 예시에 나온 속성과 이벤트를 사용합니다. 공통 방식은 [상태 제어](/ko/guides/state-ownership)에서 설명합니다.
